...
public class CartServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		ArrayList<Object> searchAL = (ArrayList<Object>) session.getAttribute("searchAL");
		ArrayList<Object> detailAL = (ArrayList<Object>)searchAL.get(detail);
		...
		if(session.getAttribute("cartAL") == null) {
			...
			for(int i = 0; i < cartAL.size(); i++) {
				...
				int itemsQty = (int) itemsAL.get(7);

				if (detailID.equals(itemsID)) {
					quantity += itemsQty;
					itemsAL.set(7, quantity);
